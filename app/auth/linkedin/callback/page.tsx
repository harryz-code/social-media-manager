...
function LinkedInCallbackContent() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        const connection = await PlatformService.handleAuthCallback('linkedin', code)
        setStatus('success')
        setMessage(`Successfully connected to LinkedIn as ${connection.profile.name}`)
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 text-center">
          {status === 'loading' && (
            <>
              ...
              <h1 className="text-xl font-semibold text-gray-900 mb-2">Connecting to LinkedIn...</h1>
